...
for i in range(len(labels)):
        ...
        for j in range(0,n_bins):
                magx = np.amax(xfourier[i][0+(spacing*j)])
                magy = np.amax(yfourier[i][0+(spacing*j)])
                magz = np.amax(zfourier[i][0+(spacing*j)])
                mags.append(magx)
                mags.append(magy)
                mags.append(magz)
        magn.append(mags)

X_train, X_test, y_train, y_test = train_test_split(magn, labels, test_size=0.40)
